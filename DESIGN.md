...
**取数**（`TaskSession.RefreshMirrorAsync`，挂在 `MainWindow.OnFrame` 的秒边界上，
**不新开定时器**——DECISIONS L8）：每秒 `GET /api/0/buckets/`（**739 字节**，一次拿到两个桶的
`last_updated`），前进了才去 `?limit=N` 拉事件——稳态下事件查询约每 10 秒才真的发生一次。
初始化用 `?limit=1&end=<镜像起点>` 拿"覆盖起点的那条"，加一次 `[起点, now]` 范围查询，
**两条加起来完备，不需要任何放宽**。
