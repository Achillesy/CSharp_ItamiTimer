...
### 9.2 这一分钟按什么顺序做事（2.0.7，DECISIONS L8~L11）
...
整分钟边界（_frame 观察到 sec == 0，实测滞后 10~24ms）
  1. 闹钟到点？ Execute 开 → 执行命令，await 到子进程退出
                Execute 关 → 记下「待响铃」，继续往下
...
- **闹钟排第 1 且 `await` 到命令退出**，但**闹钟响铃排第 4**。「先判断」和「后响铃」
  是两件事，不矛盾：响铃必须排在 Alarms 清单之后，因为 Windows 的 winmm 是单通道，
  谁后响谁截断前一个；闹钟响完什么都不留，清单响完还留着一分钟的提示条，让闹钟赢
  这一下代价更小（§9.1 沿用）。
- **命令等不到结果是正常情况**：关机/重启会把本进程一起杀掉，`await` 后面那几行根本
  不会执行。这一分钟的判定没做完也不可惜——账本本来就是下次启动时按 AW fail-closed
  重数的（§11.2）。
...
⚠️ **2.0.7 的改动（§9.2 的分钟序列、命令 `await` 到退出）在 Windows 上一次都没跑过。**
Windows 侧要重点看两处，都是这次改动新引入的：**①** 命令是 `cmd.exe /c`，`await` 子进程
退出这条路在 Windows 上的行为（尤其 `shutdown /s /t 0` 会不会立刻返回、把后面几步也跑
掉）；**②** winmm 单通道下"清单先响、闹钟后响"的截断顺序有没有因为改成顺序执行而变化。
