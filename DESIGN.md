...
**取数（App 层，尚未接线）**：每秒 `GET /api/0/buckets/`（**739 字节**，一次拿到两个桶的
`last_updated`），前进了才去 `?limit=N` 拉事件——稳态下事件查询约每 10 秒才发生一次。
...
**采样窗口：`[now-20s, now-15s)`，5 秒宽，5 秒一次。**

- **退后 15 秒是全部关键**：AW 的窗口事件滞后 `now` 约 6~12 秒（§7.2 的 T3）。窗口若贴着
  `now` 结束，尾巴上那 6~12 秒必然一条事件都没有 = `AwOffline` = 按约定**算专注**
  （§3.1 的知情 fail-open），判据就成了大半由 fail-open 决定而不是由你实际在干什么决定
  （5 秒窗口贴着 now 的话，水分可达 40%）。退后 15 秒，这 5 秒的数据已经落定。
  ⚠️ 改小到 12 以下会重新吃进那段滞后，而且**不报错**，只是越来越不敏感。
- **节拍等于宽度**：5 秒宽的窗口每 5 秒滑 5 秒 = 前后首尾相接、**无缝分块**，任何一秒都
  不漏也不重。每秒采一次结果一样，但每次查询会把过去 6 小时的事件整个拉回来再裁
  （`AwClient.FetchEventsAsync` 为绕过 T1 统一放宽 6 小时），是 5 倍开销，只换来起止时刻
  更平滑。
- 挂在 `MainWindow.OnFrame` 的秒边界上（`sec % 5 == 0`），**不新开定时器**（DECISIONS L8：
  这个程序里只有一个钟）。

**判据：这 5 秒里存在 `OffTask` 秒。**

⚠️ **绝不能写成"存在不是 `Focused` 的秒"**。扣掉 afk 后只剩三种码，而 `AwOffline`
（这一秒 AW 一条事件都没有）算专注。写成"不是 Focused 就反色"的后果是：`aw-watcher-window`
一死，整段全是 `AwOffline`，**钟面永久反色，而账本这段时间全判绿**——屏幕和账本对着说反话。
写成 `== OffTask`，watcher 死掉时它安静地不反色。

**afk 是白拿的**：`Judgment.Paint` 把 afk 画在最后、覆盖一切，所以人不在座的那些秒根本不会
以 `OffTask` 的身份留在数组里——"扣掉 afk"由现成的覆盖顺序完成，一个字都不用写。整段都是
afk 时自然没有 `OffTask`，不反色。

**复用判定，但不碰账本**（`Inversion.Evaluate`，Core，纯函数）：另开一个 **5 格的临时数组**，
初始化成 `AwOffline`，用同一个 `GroupRules`、同一个 `Judgment.Paint`、同一套覆盖顺序画一遍，
数完就扔。
⚠️ **不能改成调 `JudgmentBuffer.Cover`**：它锚死在整分钟上（DECISIONS H9），而且会写进任务
的 buffer——那是账本。

**AW 出问题一律回到底色**：查询抛异常 → 不反色；查询成功但 watcher 死了没数据 → 全
`AwOffline` → 也不反色。两条路殊途同归。理由不是"没数据不好意思判"，而是**不让提示说谎**
——AW 挂了那一分钟会被 fail-open 判成全绿。

**只在专注阶段跑**：休息（挣来的）、任务结束/放弃、空闲一律底色；结束那一刻立刻收回，不等
下一次采样。

**已接受的代价（知情）**：

1. **钟面反的是 15~20 秒前的事**。一次 1 秒的瞥视 = 15 秒后钟面反色、5 秒后自己恢复。你正
   专心写代码时看到钟面反着，读到的是"我在跑偏"，而实际上你早回来了。用户 2026-08-29 明确
   接受：要的就是"任何一次分心都要被戳一下"，闪烁是这个规则的特征不是缺陷。
2. 每 5 秒两次 AW 查询（两个 bucket），每次实际拉回过去 6 小时的事件。比原来（每分钟 2 次）
   高 12 倍。本地服务，可接受。

**日志**只在**状态变化**那一下记一行（带窗口区间），采样本身出错不记——5 秒一次会把日志淹掉，
而"AW 连不上"每整分钟那次 tick 已经记过了。
...
- **关（默认）时，安静是挣来的，不是默认值**（2026-08-13，DECISIONS C7）：喇叭图标照旧
  ...
  **`FocusLow` 不算**（2026-08-13，DECISIONS C8）：分钟刚跑完那一刻读到的判定，尾部
  几秒经常还是 `JudgmentBuffer.Cover` 的 fail-open 默认值 `AwOffline`（算作专注），要等
  下一次重绘才会被真实事件订正——这几秒的量级几乎不可能撞过 `FocusMid` 的门槛，只会
  停在 `FocusLow`，所以把它排除在外就顺手滤掉了这层噪声，不用等下一分钟订正也不用引入
  额外延迟。滴答在这里是提醒，不是惩罚：跟表盘配色那条"人不在不算你的错、不计分"
  （DECISIONS D3）是两回事——这条不做记分，纯粹是"时钟还在走"的中性信号，AFK 和切去
  做别的事一样会让下一分钟继续响，因为提醒的对象正是**无意识**的走神，不分是切窗口还是
  起身离开。
