...
- **检查节拍**：每分钟一次，在 `MainWindow.OnMinute` 那段直线代码里（DECISIONS L8/L9）。
...
### 9.2 这一分钟按什么顺序做事（2.0.7，DECISIONS L8~L12）
...
- **闹钟到点在第 1 步判断，响铃在第 4 步，命令在第 6 步**。三件事分在三处，各有各的理由：
  - **响铃排在 Alarms 清单之后**：Windows 的 winmm 是单通道，谁后响谁截断前一个；闹钟
    响完什么都不留，清单响完还留着一分钟的提示条，让闹钟赢这一下代价更小（§9.1 沿用）。
  - **命令排到最后**（2026-08-08 改，DECISIONS L12，推翻了同日早些时候的"命令优先"）。
    原来的理由是"关机会把本进程一起杀掉，`await` 后面那几行根本不会执行"——**这个前提
    只在 macOS 成立**。Windows 的 `shutdown /s /t 0` 是提交关机请求就返回，`await` 立刻
    回来，后面的 AW 查询照跑，正好落在系统正在关机的时候——**那正是本节开头说的那个
    bug，在 Windows 上等于没修**。命令放到最后，这个修复不再依赖"进程会被杀掉"这个平台
    特性，两个平台都成立。代价是关机比原来晚一点（最坏是这一分钟 AW 查询的 10 秒超时），
    对预约关机可以忽略。
- **整段只有一层 `try`，第 2~5 步不单独包**：那里面抛异常这一分钟就到此为止，第 6 步的
  命令**不会执行**，而闹钟已经 `MarkFired`（一次性），这次不跑就永远不跑——预约的关机
  不会发生。**这是知情接受的**：真出这种事，`catch` 里那行 Error 日志就是唯一也是应该被
  看到的信号。写过一版内层 `try` 让命令"就算前面炸了也照跑"，用户当场否掉：那样会把一次
  真正的故障粉饰成已处理，反而让人略过日志里那一行。
