...
| | 内容 | 落点 |
|---|---|---|
| ✅ | `during.json` 落盘：启动读入、`+=`、每次入账立刻写盘 | `During.cs` |
| ✅ | 归档当场入账（`Settled` 事件）；终结时幂等地取走剩余 | `TaskSession` / `MainWindow.Bank` |
| ✅ | 关窗口（专注中确认放弃 / 休息中直接关）也记账 | `MainWindow.OnClosing` |
...
| `during.json` | **程序**，每次入账立刻写 | 每个小目标的累计专注秒数（§11.2）|
...
### 11.2 `during`：一个小目标上花掉的总时长（2026-08-02 定义，落点待定）

用户要的语义：**一个 goal 上消耗的总秒数**。程序启动时读入，每次任务**完成或中途退出**
都把本轮专注时间加上去，程序退出时保存。

**现在三条都不对**：

- `MainWindow.EndSession` 是 `=` 不是 `+=`——所谓「累计」实际是「最近一次善终那一轮的时长」；
- 只有三条路会走到 `EndSession`（休息自然结束 / New round / 放弃确认），**关窗口退出
  一条都不记**：专注中关窗只调 `Abandon()` 写日志，休息中关窗被 `OnClosing` 第一行
  直接放行；
- 任务跑过 2 小时时，归档已经把整小时的专注秒累进 `DuringSeconds`（§4.4），落盘时
  要从这个值接着算，不能只数 buffer。
...
- **界面上显示的是 `已落盘 + 本轮未入账`（2026-08-03）**，跟着整分钟节拍刷新，
  **落盘时机一个字没变**——未入账那项是 `TaskSession.UnbankedSeconds`（只读窥视口，
  绝不能用会置位 `_banked` 的 `TakeUnbankedSeconds()`）。
  之所以敢显示，是因为**入账瞬间这个和不变**：归档加进 during 的正好是离开 buffer 的那段
  （§4.4 的 `[180,3780)`，而 `FocusedSeconds` 不含 padding，不会重复计），结束时
  `TakeUnbankedSeconds()` 把余下那份平移过去并把窥视口归零。所以交接时数字不跳。
  **它会往回退**（每拍重画最近 4 分钟，§4.2），不做单调化——见 DECISIONS D9。
- `rules.json` 因此保持**纯只读**——程序一旦写它，用户手写的注释就全部消失
  （STJ 读的时候 `JsonCommentHandling.Skip` 直接扔了，序列化时没有任何东西可以还原；
  `JsonNode` 也保不住，注释在建树之前就没了）。这正是 §11.1 判过「B 不要做」的那件事。
- 不并进 `settings.json`：那份文件程序随时整份重写，而累计时长是**唯一一个丢了就补不回来**
  的数据，单独一个文件更好备份、也更好手动清零。

**「ignore」包含三件事**（用户 2026-08-02）：

1. 用户点 Give up / 关窗口确认放弃；
2. **任务运行中直接关闭程序**；
3. **2 小时归档那次「塌缩」**——它本来就等价于「1 小时前放弃、又立刻重开」（§4.4），
   所以归档结算出来的那段专注秒数，就在那一刻 `+=` 进 `during`。

于是 `during` 的累加点是统一的：**每一秒专注时间只入账一次**，归档时入账即将被移出
buffer 的那一小时，终结时入账 buffer 里剩下的那部分。`JudgmentBuffer.Tick` 返回的
`SettledSeconds` 就是给这个用的。

**崩溃/关机仍然会丢当前这一段**（最长 2 小时，因为归档会中途落一次）。这跟 B1
「退出即放弃、不落盘」是同一条纪律：任务状态不落盘，只有这个纯统计量落盘。
...
**已经一处都没有了**（2026-08-02 核查）：`AwClient.ProbeAsync` 全仓库没有调用点
（只有 CLI 用），`FindBucketIdAsync` 只在第一个计时点惰性触发。启动与退出路径上零 AW 访问。
