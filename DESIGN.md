...
- **到点做两件事**：
  ...
  2. **受 Settings 的 Alarms 开关控制**是否也出声（`Sound.Repeat`，2 遍，独立音色）。
     这个开关是普通持久化设置，**不像 Execute 那样每次启动强制复位**——它不武装
     任何危险的东西，只管"响不响"。
- **提醒的呈现方式：程序自己画一条提示条，不是 Windows 系统通知**（2026-08-06
  推翻，方案 B）。最初实现走的是 Windows 原生 toast——起一个短命的 `powershell.exe`
  子进程调 WinRT 的 `ToastNotificationManager`。**真机验证发现这条路完全走不通**：
  `powershell.exe` 用的 AppId 在这台 Windows 上从没注册过，API 调用不报错、
  `Show()` 正常返回，但通知被**静默丢弃**——Windows 设置里"通知"的应用列表中根本
  找不到 PowerShell，连人工开关都没有，试了两种常见 AppId（裸字符串
  `"Windows PowerShell"` 和社区惯用的
  `{1AC14E77-02E7-4E5D-B744-2EB1AE5198B7}\WindowsPowerShell\v1.0\powershell.exe`）
  结果一样。真要走通，得给程序注册一个真正的 AppUserModelID（装机时放一个
  properties 齐全的开始菜单快捷方式），成本和不确定性都不小，于是改道：**程序
  自己在骨牌所在的 Grid 里叠一层内容**，跟骨牌共用同一块屏幕区域和尺寸，不单开
  窗口、不碰操作系统的通知机制。`Platform/Notify.cs` 已删除。
  ⚠️ **代价**：不再有"关掉程序也能事后翻看"的系统通知历史——提醒只在那一分钟
  内、程序还开着的时候可见。这唯一的事后痕迹变成了 `itami.log` 里
  `Alarms 清单到点: ...` 那一行，跟这个项目其它地方"界面沉默、日志是唯一能事后
  查的地方"是同一个待遇，不是新豁免。
