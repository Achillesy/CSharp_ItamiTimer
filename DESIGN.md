...
- **到点做两件事**：
  1. **无条件**弹一条系统通知，正文就是清单那一行的原文（`Notify.Show`）；
  2. **受 Settings 的 Alarms 开关控制**是否也出声（`Sound.Repeat`，2 遍，独立音色）。
     这个开关是普通持久化设置，**不像 Execute 那样每次启动强制复位**——它不武装
     任何危险的东西，只管"响不响"。
- **系统通知的实现**（`Platform/Notify.cs`）：起一个短命子进程，不接原生
  WinRT/UWP 绑定、不加包依赖。Windows 上直接调 WinRT 的
  `ToastNotificationManager`，整段脚本 Base64 编码后走 `powershell.exe
  -EncodedCommand`；macOS 上 `osascript -e 'display notification'`。失败只写日志，
  通知本身绝不能把程序搞挂。
