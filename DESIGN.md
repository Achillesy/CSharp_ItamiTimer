...
### 13.1 ⚠️ 2.0.x 的无边框透明窗口：macOS 侧**全部未验证**

§8.7 那一整套（无边框、透明、拖动、右键菜单、位置记忆与归位）**只在 Windows 真机上
验证过**，一次都没在 macOS 上跑过。用的都是 Avalonia 的跨平台 API，按文档三个桌面
后端都支持，但这个项目已经栽过一次"照文档推断、编译通过、功能安静失效"（H12 的滚轮
`Delta/120`），所以**在 macOS 上要当成待验证清单逐条过，不要因为编译过就认为通了**：

| 要验证的 | 怎么算通过 | 已知的 macOS 侧差异 |
|---|---|---|
| `WindowDecorations="None"` + 透明背景 | 窗口没有标题栏、没有底色，桌面透出来 | 透明合成走 Quartz 不是 DWM，**边缘抗锯齿/圆角观感未必跟 Windows 一样**，要用眼睛看 |
| `BeginMoveDrag` 拖动 | 按住表盘能拖动整扇窗口，手感跟拖标题栏一致 | Avalonia 文档说支持，本项目未测 |
| 命中范围是圆的 | 表盘四角拖不动、圆盘边缘外一点能拖 | 命中测试在 Avalonia 层，理论上与平台无关，但值得顺手确认 |
| 右键菜单 | 表盘上右键出 `Close window`，点了走放弃确认 | macOS 右键/双指点按 |
| `ClampIntoScreen` | 拖出屏幕松手后弹回可见区域 | **`WorkingArea` 在 macOS 要扣掉顶部菜单栏和 Dock**，这条正是最可能出偏差的地方 |
| 位置记忆 | 关掉重开回到上次的位置 | `Position` 在 macOS 上的坐标原点/翻转问题（Cocoa 的 Y 轴朝上）由 Avalonia 抹平，但**没实测过** |
| 置顶（图钉） | 点图钉窗口压在其它窗口之上 | 这条**有本项目自己的实测背书**：macOS 走 `Window.Topmost`（不带激活语义），见 `WindowPin.cs` 的注释 |

Windows 侧已经实测过的两条，可以照抄验证方法：把 `settings.json` 的 `windowX`/`windowY`
