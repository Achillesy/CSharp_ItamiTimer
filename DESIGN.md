...
bucket 一旦建过就永远存在，所以 watcher 死了照样找得到。堵法（某个桶在 4 分钟窗口里
零事件即视为 AW 异常）见 §16.5，用户 2026-08-02 明确**暂不处理**。
...
4. §15.2（Give Up 后按钮不恢复）没动，那是 Avalonia 的控件父子关系问题，跟引擎无关。
...
一点提示都没有。** 正确范围是 `[180, 3780)`，推导见 §4.4。改一行。

## 15.6 ⚠️ 机器睡眠超过 2 小时 3 分 → 会话永久冻死

`JudgmentBuffer.Write` 里 `len = Min(240, 7380 − bufferOffset)`，`bufferOffset ≥ 7380`
时 `len ≤ 0` 直接 return，`ElapsedSeconds` 不再前进 → `TryArchive` 的条件永远不成立 →
表盘冻住、达成永远不来、只能放弃。整个过程没有任何日志和提示。

`bufferOffset < 0` 也没有检查，现在恰好安全，但那是**靠 `startedAt` 截断到整分钟撑着的**
（A6），哪天改回进位就当场越界。
...
还在讲退化模式（degraded mode / plain pomodoro）、多选 checkbox、`ignore` 名单、
「中途补勾追溯生效」、50 分钟上限、tomato icon。这些全部已经删除或改掉。
它是**唯一对外可见**的文档，用户看见的行为变了要同步（CLAUDE.md 的规矩）。
...
## 16.5 AW 异常检测（用户 2026-08-02：暂不处理，记录备查）
...
一条检查能同时堵住：**某个桶在这 4 分钟查询窗口里返回零事件 → 视为 AW 异常**，
整拍按「连不上」处理。误判方向是善意的（算专注），而连续 4 分钟一个事件都没有本身
就不正常。

另一个半独立的问题：**提交任务时要不要探活**。中途断线算专注是合理的（不该拿 AW 的
毛病惩罚用户），但**开局就没有 AW** 是另一回事——那样整个约束从一开始就形同虚设。
`ProbeAsync` 现成的（§6.2 原本就要求「连不上就拒绝提交」），只是没人调用。
