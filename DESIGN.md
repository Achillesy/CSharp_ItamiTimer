...
（AW 只按事件自己的 start 过滤，跨起点的长事件静默消失）**无条件把查询往前放宽 6 小时**
——一次拉回 **337 条 / 65.7 KB**，只为裁出其中 4 分钟。⚠️ 而且那 6 小时**既是猜的、又
不可证充分**：一个窗口连续保持同一标题超过 6 小时（挂机、长视频、盯着一个 PDF 过夜），
照样会被丢掉，方向是 fail-open 所以从来没人发现。**这个洞跟镜像是两件事，单独记着。**
...
### 8.9 跑偏就把钟面反色（3.1.0 落地，3.2.0 撤下待重做）

> ⚠️ **3.2.0 把实现整个删掉了**（`Inversion.cs`、采样、`WithFaceFrom` 半反色调色板）。
> 原因不是设计错了，是**效果不对**：用户 2026-08-29 装机实测后判定"5 秒一次的反转对
> 视觉的影响太小，需要一个频繁闪烁的东西才能吸引注意力"。重做的方向已经想清楚：
> 判据从 AW 内存镜像（§7.5）读，**1 秒粒度**；而"闪烁"是**纯渲染**，跟采样频率无关，
> 一次 AW 查询都不需要。下面这一节的其余内容（翻哪几个色号、为什么色环不能跟着翻、
> 为什么 AW 挂了不能反色）重做时照用。


专注期间，**最近那一小段里出现过跑偏，钟面就翻到另一档**；没有就用用户设定的底色。
不弹窗、不出声——跟表盘上的红格和越滑越远的截止弧同一条路数（§2 原则）。
...
var wanted = !session.InRest && second.Code == JudgmentCode.OffTask;
```
...
- **关（默认）时，安静是挣来的，不是默认值**（2026-08-13，DECISIONS C7）：喇叭图标照旧
  手动开关滴答（`TickEnabled` 关着就永远不响，人为选择优先级最高，没有任何情况能覆盖）；
  `TickEnabled` 开着时**默认响**，跟一个普通时钟一样——空闲、休息、任务刚开始还没出
  第一次判定、或者上一整分钟被判定为脱离任务，都响。**唯一能换来安静的，是"当前在
  专注阶段 + 上一分钟 `MinuteCell.Tier` 是 `FocusFull` 或 `FocusMid`"这一种情况**——
  **`FocusLow` 不算**（2026-08-13，DECISIONS C8）。滴答在这里是提醒，不是惩罚：跟表盘
  配色那条"人不在不算你的错、不计分"（DECISIONS D3）是两回事——这条不做记分，纯粹是
  "时钟还在走"的中性信号，AFK 和切去做别的事一样会让下一分钟继续响，因为提醒的对象
  正是**无意识**的走神，不分是切窗口还是起身离开。
