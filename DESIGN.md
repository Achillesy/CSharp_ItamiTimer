...
- **关（默认）时，安静是挣来的，不是默认值**（2026-08-13，DECISIONS C7）：喇叭图标照旧
  ...
  第一次判定、或者上一整分钟被判定为脱离任务（`MinuteCell.Tier` 不是 `FocusFull/Mid/Low`），
  都响。**唯一能换来安静的，是"当前在专注阶段 + 上一分钟真被判定为专注"这一种情况**。
  滴答在这里是提醒，不是惩罚：跟表盘配色那条"人不在不算你的错、不计分"（DECISIONS D3）
  是两回事——这条不做记分，纯粹是"时钟还在走"的中性信号，AFK 和切去做别的事一样会
  让下一分钟继续响，因为提醒的对象正是**无意识**的走神，不分是切窗口还是起身离开。
