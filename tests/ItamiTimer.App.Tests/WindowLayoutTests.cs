...
public class WindowLayoutTests
{
    ...
    [Fact]
    public void 紧凑档的齿轮那一排必须整个抬出骨牌行()
    {
        // ⚠️ 用户 2026-09-05 实机发现的毛病，而且是「看星期几」的：图标固定 28px 不缩放，
        // 紧凑档骨牌行只有 56px，周一到周三右边那块还立着/正在倒的骨牌会被齿轮压住，
        // 周四以后倒平了反而不遮——时有时无，最难查的那一类。
        //
        // **标准档不设这条**：76px 的行里图标只占 37%，而右边那几块本来就是倒下的、很矮，
        // 纵向虽然叠着但横向不撞；而且齿轮压在骨牌上方是 D6 故意要的观感，不能拿这条去否它。
        //
        // 紧凑档的判据是"整排抬出骨牌行"：那一排挂在骨牌行上、Top 对齐，所以下沿
        // = ChromeRowMargin.Top + 28，必须不越过骨牌行的上边界（0）。
        var m = WindowLayout.Compact;
        var bottom = m.ChromeRowMargin.Top + 28;
        Assert.True(bottom <= 0, $"齿轮那一排的下沿在 {bottom}，还留在骨牌行里，会压到骨牌");
    }
