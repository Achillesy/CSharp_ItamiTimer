...
3.1.0 起**跑偏时钟面会翻到另一档**。专注期间每五秒看一眼 ActivityWatch 的历史——取的是
**十五秒之前**结束的那五秒（那时候数据已经落定了），只要其中有一秒你不在目标里，盘面、
刻度、指针就换成相反的那一档。木边框、色环、卡片以及其它窗口**全都不动**：这是提示，
不是换主题。跟这里其它东西一样是沉默的。

两件事得先知道。**它显示的是刚过去的二十秒，不是此刻**——瞄一眼聊天窗口一秒钟，十五秒后
钟面才翻，那时你早回来了，再过五秒它自己恢复。以及 **ActivityWatch 挂掉时钟面保持你设定
的样子**：那段时间账本本来就按专注算，一个跟账本对着说话的提示比没有提示更糟。
