...
程序到点把命令**发出去就立刻返回**，全程不等——命令挂住也卡不到时钟。两个平台达到
这个效果的方式不同，**理由并不对称**。

**Windows** 上程序开一个 PowerShell 窗口（`-NoExit`），让**那个窗口**去跑
`itami commands --execute --yes`。这一层绕路只为一个原因：**有些命令只把失败讲给控制台听。**
`shutdown /h` 在没启用休眠的机器上退出码 0、stdout 和 stderr 都是空的、机器纹丝不动——
"此系统上没有启用休眠"那句话直接写给了控制台，重定向的管道永远看不到。窗口故意留着不关，
你回来时那句话还在。

**macOS** 上程序直接把命令跑起来，退出码、stdout、stderr 全部写进 `itami.log`。macOS
没有那种"只讲给控制台"的毛病：`pmset -x`、`shutdown`、`open /nonexistent`、不存在的命令，
失败信息都老老实实走 stdout/stderr，管道抓得到。所以这边不弹窗，也就不需要为了弹窗去写
临时脚本、套 Terminal 那一串。
