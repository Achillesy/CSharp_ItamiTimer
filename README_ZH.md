...
程序**不自己执行**你的命令。它开一个 shell 窗口，让**那个窗口**去跑
`itami commands --execute --yes`，然后就撒手不管了——不捕获输出、不看退出码、不等待。

绕这一圈只为一件事：**有些命令只会把失败讲给控制台听。** 在没启用休眠的机器上，
`shutdown /h` 退出码是 0、stdout 和 stderr 都是空的、机器纹丝不动——那句"此系统上没有
启用休眠"是**直接写给控制台**的，被管道捕获的进程永远收不到。放在一个真实窗口里跑，
你直接就能读到哪里出了问题。
