...
结果 **27 MB / 35 个文件**。`dotnet publish` 本身**不剔除调试符号**（它只把 NuGet 原生包照搬出来），原样输出是 127 MB —— 差额就 `libSkiaSharp.pdb` 80.1 MB + `libHarfBuzzSharp.pdb` 19.9 MB 两个文件。csproj 里挂了个 `StripPdbFromPublish` 目标替你删掉了，两个平台都生效。
