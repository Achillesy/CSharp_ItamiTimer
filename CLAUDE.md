...
## ⚠️ 平台验证进度：macOS 过了一轮，2.0.7 的改动 Windows 还没跑过
...
**反过来，2.0.7 的改动（DESIGN §9.2：所有分钟级功能收成一条直线、命令 `await` 到子
进程退出）只在 macOS 上跑过，Windows 一次都没有。** Windows 侧重点看两处：`cmd.exe /c`
下 `await` 子进程退出的行为（`shutdown /s /t 0` 会不会立刻返回），以及 winmm 单通道下
"清单先响、闹钟后响"的截断顺序有没有变。
