...
- **⚠️ 不要再把「置顶提醒」写回来。** 2026-07-28 用户明确砍掉整套机制（原 §8.3）：「不要再纠结窗口置顶这种事情了。逻辑混乱，又容易出错。」`Win32Topmost.cs` 已删除。被否掉的是**表达方式**，不是那些检测本身——同一天用户就把键鼠空闲检测加了回来，只是改用声音（§8.3.4 的那段引言）。
...
- **提示音 + 设置窗口**（§8.3.1~8.3.3）是 2026-07-28 新进 v1 的，已实现。三条声音，各自可关。
