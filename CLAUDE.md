...
2.0.0~2.0.4 把主窗口整个改成了无边框 + 透明背景（拖表盘移动、右键关窗、记住位置、
拖出屏幕自动归位），**全部只在 Windows 真机上验证过**。规格见 DESIGN §8.7，
**macOS 待验证清单见 DESIGN §13.1**——那张表列了每一条怎么算通过、以及哪几处最可能
出偏差（`WorkingArea` 在 macOS 要扣掉顶部菜单栏和 Dock；透明合成走 Quartz 不是 DWM，
观感未必一致）。用的都是 Avalonia 跨平台 API、编译也没问题，但这个项目正是在
「照文档推断 → 编译通过 → 功能安静失效」上栽过（H12），**别把编译通过当验证通过**。
