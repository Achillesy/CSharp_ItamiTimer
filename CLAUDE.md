...
- **⚠️ 不要再把「置顶提醒」写回来。** 2026-07-28 用户明确砍掉整套机制（原 §8.3）：「不要再纠结窗口置顶这种事情了。逻辑混乱，又容易出错。」`Win32Topmost.cs` 和 `InputIdle.cs` 已删除。**连带砍掉的还有键鼠空闲督促**——所以"安静读 PDF 超过 180 秒被判 Absent"这个已知代价现在连提醒都没有了，这是用户知情后的选择（§8.3.4）。
- **唯一需要 Windows 专属实现的地方**：`winmm.dll` 的 `PlaySound`，放系统自带的 wav（§8.3.1）。只用 `C:\Windows\Media` 里的文件，不打包音频资源——跟表盘不用位图是同一条纪律。
- **提醒只剩两声**：专注达成、休息结束，都能在设置里关。**跑偏没有任何主动提醒，只写一行日志。**
...
- **提示音 + 设置窗口**（§8.3.1~8.3.3）是 2026-07-28 新进 v1 的，已实现。
